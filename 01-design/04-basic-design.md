...
| サービス | 責務 |
|----------|------|
| `ExpCalculator` | `基本EXP × 難易度倍率 × ストリーク倍率`、同日2回目減算、日次上限 |
...
**UI/UXモック:** `01-design/screen/` に各画面のモックを配置（今後作成）

| ID | 画面 | 目的 | 主要要素 | 遷移発火 |
|----|------|------|----------|----------|
| SC-01 | スプラッシュ | 起動・データ読込 | ロゴ、ローディング | 初回→SC-02 / 2回目以降→SC-03 |
| SC-02 | オンボーディング | 初回導線 | キャラ紹介、秒数＝EXP 説明、秒数設定、初回プランクへ | 完了→SC-08→SC-03 |
| SC-03 | ホーム | 日常の起点 | **ストリーク（最上位）**、今日の状態、キャラ、EXP バー、最長記録、次目標、プランク開始 | 開始→SC-05、カレンダー→SC-04、設定→SC-09 |
| SC-04 | ストリークカレンダー | 履歴 | 月間カレンダー、実施マーク、最長記録、称号（3/7/14/30） | 戻る→SC-03 |
| SC-05 | 種目選択 | 種目選択 | ベーシック・ニープランク2種、倍率・説明 | 選択→SC-06 |
| SC-06 | 秒数設定 | 目標秒数 | スライダー/プリセット(10〜120秒)、獲得 EXP 見込み | 開始→SC-07 |
| SC-07 | プランク実行 | 計測 | タイマー、キャラ同期、一時停止、応援セリフ | 完了→SC-08、中断→SC-03（確認ダイアログ） |
| SC-08 | 結果 | 報酬 | ストリーク+1演出、EXP 内訳、マイルストーン、レベルアップ、進化 | 閉じる→SC-03 |
| SC-09 | 設定 | 設定 | デフォルト秒数、通知 ON/OFF（任意）、データリセット | 戻る→SC-03 |
...
| 優先度 | 要素 | 仕様 |
|--------|------|------|
| 1 | 現在ストリーク | 大きな数字 + 炎アイコン。「○日連続！」 |
| 2 | 今日の状態 | 未実施：「今日のプランクでストリーク維持」／完了：チェック |
| 3 | 次の目標 | 次の倍率段階 or マイルストーンまで「あと○日」 |
| 4 | キャラ | 腹筋ステージ・ストリーク連動のポーズ/表情 |
| 5 | EXP バー | 現在レベル、次レベルまでの進捗 |
| 6 | 最長ストリーク | 個人ベスト常時表示 |
| 7 | CTA | 「プランク開始」ボタン（画面下部・固定） |
...
| I/F | 入力 | 出力 |
|-----|------|------|
| `ExpCalculator.calculate` | `baseExp`, `difficultyMultiplier`, `streakMultiplier`, `isSecondSessionToday` | `int`（獲得EXP） |
...
sequenceDiagram
    ...
    UC->>Exp: calculate(30, 1.0, streakMult, is2ndSession)
...
**MVP DoD との対応:** 本設計の SC-01〜09、CompletePlankUseCase、ApplyDailyPenaltyUseCase、ローカル Repository で [`03-phase.md`](03-phase.md) の DoD を満たす。
...
*最終更新: 2026-06-10 / バージョン: 1.0（MVP 基本設計）*

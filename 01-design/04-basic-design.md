...
| `PlankTimer` | 開始前 3・2・1 カウント、目標秒数カウントダウン、一時停止／再開、バックグラウンド経過時間の補正 |
...
| SC-04 | 記録（記録タブ） | 履歴・振り返り | 月間カレンダー、**現在／最長ストリーク**、**日付タップで日別実施履歴** | タブ→SC-03 / SC-10 / SC-09 |
| SC-05 | ~~種目選択~~ | — | **SC-03 種目ビューに統合** | — |
| SC-06 | ~~秒数設定~~ | — | **SC-03 種目ビューに統合** | — |
| SC-07 | プランク実行 | 計測 | 3・2・1 開始カウント → 秒数タイマー自動開始。計測中は一時停止のみ。停止中は再開・終了 | 0秒→SC-08、終了→SC-03 |
| SC-07b | プランクセット実行 | 連続計測 | SET-01 またはカスタムセットを順番に SC-07 相当で実行。種目間「次へ」 | 全種完了→SC-08、中断→SC-03 |
| SC-11 | カスタムセット編集 | セット作成・編集 | セット名、種目の追加・並べ替え・削除、種目ごとの秒数、保存／削除 | 保存→SC-03、キャンセル→SC-03 |
| SC-12 | カスタム種目編集 | 単種目の作成・編集 | 種目名入力、ベーシックプランク固定プレビュー、保存／削除 | 保存→SC-03、キャンセル→SC-03 |
...
| パネルB（種目ビュー） | 種目カルーセル | **公式単種目** → **カスタム種目** → **＋（種目）** → SET-01 → **カスタムセット** → **＋（セット）** の順でスワイプ |
| パネルB（種目ビュー） | カスタム種目 | ユーザー名を表示。ポーズは PK-01 固定。秒数 `− / +`・スタートで SC-07 |
| パネルB（種目ビュー） | ＋（種目） | タップで SC-12（新規カスタム種目）。既存カスタム種目は編集アイコン等で SC-12 |
| パネルB（種目ビュー） | セット表示 | SET-01 / カスタムセット選択時：セット名・種目一覧・種目ごとの秒数 `− / +` |
| パネルB（種目ビュー） | ＋（セット） | カルーセル末尾。タップで SC-11（新規セット作成） |
| パネルB（種目ビュー） | 秒数 | 5秒刻み `− / +`（10〜120秒）。セットは種目ごとに個別 |
| パネルB（種目ビュー） | CTA | `▷ スタート` → 単種目 SC-07 / セット SC-07b |
...
| フェーズ | 表示 | 操作 |
|----------|------|------|
| `preparing` | **3 → 2 → 1**（全画面またはタイマー領域に大表示） | なし |
| `running` | 残り秒数、種目ポーズ、応援セリフ | **一時停止** のみ |
| `paused` | 残り秒数（停止中表示） | **再開**・**終了** |
| `completing` | ローディング | なし |
...
SC-03 スタート
  ...
  → running（自動）
  → paused（一時停止タップ）
  → running（再開）/ SC-03（終了・0 EXP）
  → SC-08（0 秒到達・自動完了）
```

**廃止:** 「開始」「完了」ボタン（手動）。0 秒到達で `CompletePlankUseCase` を実行。

#### SC-04 記録

| 領域 | 要素 | 仕様 |
|------|------|------|
| 上部 | 月間カレンダー | 実施日マーク。日付タップで選択（ハイライト） |
| 中部 | ストリーク | **現在のストリーク**・**最長ストリーク** |
| 下部 | 日別履歴 | 選択日の `WorkoutRecord` 一覧（`WorkoutRepository.getByDate`）。種目名・秒数・EXP・完了時刻・同日回数 |
| 初期選択 | 今日 | 起動時は今日を選択。未実施なら空状態メッセージ |
| 下部タブ | ナビゲーション | home / 記録（現在地）/ アルバム / 設定 |

#### SC-07b プランクセット実行

| 項目 | 仕様 |
|------|------|
| セット | SET-01（固定）または **カスタムセット**（`CustomPlankSet`） |
| UI | 現在 N/M 種目、種目名。**各種目で SC-07 と同一 UI**（3・2・1 → 計測 → 一時停止／再開・終了） |
| 種目間 | 1種目完了 → 「次の種目へ」→ 2種目… |
| 完了 | 全種目完了後 SC-08（合算 EXP・各種目内訳） |
| データ | 種目ごとに `WorkoutRecord` を保存。種目ごとの `targetSeconds` はセット定義に従う |

#### SC-11 カスタムセット編集

| 領域 | 要素 | 仕様 |
|------|------|------|
| ヘッダー | セット名 | テキスト入力（必須・最大30文字程度） |
| 中部 | 種目リスト | 追加済み種目を**上から順に**表示。各行：種目名・秒数 `− / +`・削除 |
| 中部 | 種目追加 | **＋ 種目を追加** → 利用可能な**公式種目＋カスタム種目**から選択（ロック種目は選択不可） |
| 中部 | 並べ替え | ドラッグハンドル or 上下ボタンで順序変更 |
| 下部 | 保存 | バリデーション通過後 `CustomPlankSetRepository.save` → SC-03 |
| 編集時 | 削除 | 確認ダイアログ後に削除 → SC-03 |

**バリデーション:** セット名未入力・種目2未満・9種超・同一種目重複は保存不可。

#### SC-12 カスタム種目編集

| 領域 | 要素 | 仕様 |
|------|------|------|
| 上部 | プレビュー | PK-01 と同一のキャラポーズ・フォーム画像（**変更不可**） |
| 中部 | 種目名 | テキスト入力（必須・最大30文字程度） |
| 下部 | 保存 | バリデーション通過後 `CustomPlankTypeRepository.save` → SC-03 |
| 編集時 | 削除 | 確認ダイアログ後に削除 → SC-03 |

**バリデーション:** 種目名未入力・前後空白のみ・20件超は保存不可。同名は許可（ID で区別）。

**ランタイム:** 保存データは `PlankType` にマージして表示。`expMultiplier: 1.0`、`poseAssetId: basic_plank`（PK-01 準拠）。
...
| | `getByDate(date)` | `Date` | `List<WorkoutRecord>` | 同日複数回判定・記録タブ日別一覧 |
...
| `CompletePlankSetUseCase` | `execute()` | `PlankSetDefinition`, `targetSecondsList` | `PlankSetResult` | SET-01 / カスタムセットの連続実行 |
| `CustomPlankSetRepository` | `getAll()` / `save()` / `delete()` | `CustomPlankSet` | — | カスタムセットのローカル永続化 |
| `CustomPlankTypeRepository` | `getAll()` / `save()` / `delete()` | `CustomPlankType` | — | カスタム種目のローカル永続化 |
| `PlankTypeRepository`（合成） | `getAll()` / `getById()` | — | `PlankType` | アセットマスタ＋カスタム種目をマージ |
...
// 実行時は PK-01 相当の PlankType に変換（poseAssetId, expMultiplier 等）
```
...
| 一時停止 | 許可。再開でタイマー続行。**0 秒到達で自動完了**（手動完了ボタンなし） |
...
*最終更新: 2026-06-12 / バージョン: 1.5（カスタム種目 SC-12 / PL-31）*

...
| SC-07b | プランクセット実行 | 3種連続計測 | SET-01（PK-01→02→03）を順番に SC-07 相当で実行。種目間「次へ」 | 全種完了→SC-08、中断→SC-03 |
...
| パネルB（種目ビュー） | 種目カルーセル | 「種目を選ぶ」、単種目（9種）と **プランクセット（SET-01）** をスワイプ選択 |
| パネルB（種目ビュー） | プランクセット表示 | SET-01 選択時：3種の並び（ベーシック→ニー→ハイ）をプレビュー表示 |
| パネルB（種目ビュー） | 秒数 | 目標秒数スライダー（10〜120秒）。セット時は全種目共通 |
...
| 項目 | 仕様 |
|------|------|
| セット | SET-01: PK-01 → PK-02 → PK-03 |
| UI | 現在 N/3 種目、種目名。**各種目で SC-07 と同一 UI**（3・2・1 → 計測 → 一時停止／再開・終了） |
| 種目間 | 1種目完了 → 「次の種目へ」（短い休憩可）→ 2種目… |
| 完了 | 3種目完了後 SC-08（合算 EXP・各種目内訳） |
| データ | 種目ごとに `WorkoutRecord` を保存。`CompletePlankSetUseCase` または SC-07 ループ |
...
| `CompletePlankSetUseCase` | `execute()` | `setId`, `targetSeconds` | `PlankSetResult` | SET-01 等の連続実行。種目ごと EXP・履歴保存、セット結果を集約 |
...
*最終更新: 2026-06-11 / バージョン: 1.3（プランク実行：3・2・1 開始・一時停止 UI）*
